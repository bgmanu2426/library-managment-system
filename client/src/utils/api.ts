...
export const validatePaginationParams = (skip: number, limit: number): { skip: number; limit: number } => {
...
export const login = async (email: string, password: string): Promise<{ success: boolean; token?: string; user?: User; error?: string }> => {
  try {
    // Validate inputs
    if (!email || !password) {
      return {
...
export const userLogin = async (email: string, password: string): Promise<{ success: boolean; token?: string; user?: User; error?: string }> => {
  return login(email, password);
};

export const adminLogin = async (email: string, password: string): Promise<{ success: boolean; token?: string; user?: User; error?: string }> => {
  return login(email, password);
};
...
export const logout = async (token: string): Promise<{ message: string; user_id?: number; timestamp?: string }> => {
  try {
...
export const getUsers = async (token: string, skip = 0, limit = 50, search?: string, role?: string): Promise<{ users: User[]; total: number }> => {
  try {
...
export const createUser = async (token: string, userData: UserCreatePayload): Promise<{ message: string; user: User }> => {
  return apiRequest<{ message: string; user: User }>(
    getApiUrl(API_ENDPOINTS.USERS),
    {
      ...createAuthenticatedRequest(token),
      method: 'POST',
...
export const updateUser = async (token: string, userId: number, userData: UserUpdatePayload): Promise<{ message: string; user: User }> => {
  return apiRequest<{ message: string; user: User }>(
...
export const getBooks = async (token: string, skip = 0, limit = 50): Promise<{ books: Book[]; total: number }> => {
  try {
...
export const createBook = async (token: string, bookData: BookCreatePayload): Promise<{ message: string; book: Book }> => {
  return apiRequest<{ message: string; book: Book }>(
    getApiUrl(API_ENDPOINTS.BOOKS),
    {
      ...createAuthenticatedRequest(token),
      method: 'POST',
...
export const updateBook = async (token: string, bookId: number, bookData: BookUpdatePayload): Promise<{ message: string; book: Book }> => {
  return apiRequest<{ message: string; book: Book }>(
...
export const searchBooks = async (token: string, query: string, skip = 0, limit = 50): Promise<{ books: Book[]; total: number }> => {
  try {
    ...
    const response = await apiRequest<Book[] | { books?: Book[]; total?: number }>(
      getApiUrl(`${API_ENDPOINTS.BOOKS}?search=${encodeURIComponent(query.trim())}&skip=${skip}&limit=${limit}`),
      createAuthenticatedRequest(token)
    );
...
export const createRack = async (token: string, rackData: RackCreatePayload): Promise<{ message: string; rack: Rack }> => {
...
export const updateRack = async (token: string, rackId: number, rackData: RackUpdatePayload): Promise<{ message: string; rack: Rack }> => {
  return apiRequest<{ message: string; rack: Rack }>(
...
export const getShelves = async (token: string, rackId?: number): Promise<{ shelves: Shelf[]; total: number }> => {
  try {
...
export const createShelf = async (token: string, shelfData: ShelfCreatePayload): Promise<{ message: string; shelf: Shelf }> => {
  return apiRequest<{ message: string; shelf: Shelf }>(
    getApiUrl(API_ENDPOINTS.SHELVES),
    {
      ...createAuthenticatedRequest(token),
      method: 'POST',
...
export const updateShelf = async (token: string, shelfId: number, shelfData: ShelfUpdatePayload): Promise<{ message: string; shelf: Shelf }> => {
  return apiRequest<{ message: string; shelf: Shelf }>(
...
export const issueBook = async (token: string, issueData: IssueBookPayload): Promise<{ message: string; transaction: Transaction }> => {
...
export const getRecentActivity = async (token: string): Promise<{ recent_activities: any[] }> => {
  try {
    ...
    if (Array.isArray(response)) {
      ...
      const activities = Array.isArray(response.recent_activities) ? response.recent_activities : [];
...
export const updateUserProfile = async (token: string, profileData: UserProfileUpdatePayload): Promise<{ message: string; user: User }> => {
  return apiRequest<{ message: string; user: User }>(
    getApiUrl(API_ENDPOINTS.USER_PROFILE),
    {
      ...createAuthenticatedRequest(token),
...
export const getAvailableBooks = async (token: string, skip = 0, limit = 10): Promise<{ books: BookResponse[]; total: number }> => {
...
export const searchAvailableBooks = async (token: string, query: string, skip = 0, limit = 10): Promise<{ books: BookResponse[]; total: number }> => {
  return apiRequest<{ books: BookResponse[]; total: number }>(
    getApiUrl(`${API_ENDPOINTS.USER_SEARCH_BOOKS}?query_text=${encodeURIComponent(query)}&skip=${skip}&limit=${limit}`),
    createAuthenticatedRequest(token)
  );
};

export const getBookHistory = async (token: string, skip = 0, limit = 10, statusFilter?: string): Promise<{ history: BookHistoryResponse[]; total: number }> => {
...
export const getCurrentBooks = async (token: string): Promise<{ books: any[] }> => {
  try {
    ...
    if (response && typeof response === 'object') {
      const books = Array.isArray(response.books) ? response.books : 
                   Array.isArray(response.current_books) ? response.current_books : [];
      return { books };
    } else {
      return { books: [] };
    }
...
export const getUserShelves = async (token: string, rackId?: number): Promise<{ shelves: Shelf[]; total: number }> => {
  try {
...
export const getOverdueBooks = async (token: string): Promise<{ overdue_books: OverdueBookResponse[] }> => {
  try {
    validateAuthToken(token);
    ...
    if (error instanceof Error) {
      if (error.message.includes('timeout')) {
        throw new Error('Request timed out while fetching overdue books. Please check your connection and try again.');
        ...
        throw new Error('Network error while fetching overdue books. Please check your internet connection.');
      }
    }
...
export const getFines = async (token: string, statusFilter?: string): Promise<{ fines: FineResponse[] }> => {
  try {
    validateAuthToken(token);
...
export const calculateFines = async (token: string, fineData: CalculateFinesPayload): Promise<{ message: string; fines_created: any[] }> => {
  try {
    validateAuthToken(token);
    ...
    if (fineData.fine_per_day === undefined || fineData.fine_per_day <= 0 || fineData.fine_per_day > 1000) {
...
export const payFine = async (token: string, fineId: number, paymentData: PayFinePayload): Promise<{ message: string; fine_id: string; payment_method: string; paid_at: string; paid_by_admin: string }> => {
  try {
    validateAuthToken(token);
    ...
    if (paymentData.notes && typeof paymentData.notes === 'string' && paymentData.notes.length > 1000) {
    ...
    return await overdueApiRequest(
      ...
      () => apiRequest<{ message: string; fine_id: string; payment_method: string; paid_at: string; paid_by_admin: string }>(
        getApiUrl(url),
        {
          ...createAuthenticatedRequest(token),
          method: 'PUT',
          body: JSON.stringify({
            ...
            notes: paymentData.notes?.trim() || `Paid via ${paymentData.payment_method || 'cash'} at library counter`
          }),
        },
    ...
    if (error instanceof Error) {
      if (error.message.includes('timeout')) {
        throw new Error('Payment processing timed out. Please verify if the payment was completed or try again.');
      } else if (error.message.includes('Authentication') || error.message.includes('401')) {
        throw new Error('Your session has expired. Please log in again to process the payment.');
      } else if (error.message.includes('403')) {
        throw new Error('You do not have permission to process payments. Please contact an administrator.');
      } else if (error.message.includes('404')) {
        throw new Error('The fine record was not found. It may have already been processed or deleted.');
      } else if (error.message.includes('400')) {
        throw new Error('This fine cannot be paid. It may already be paid, waived, or in an invalid state.');
      } else if (error.message.includes('422')) {
        throw new Error('Invalid payment information provided. Please check your payment details and try again.');
      } else if (error.message.includes('Network') || error.message.includes('fetch')) {
        throw new Error('Network error occurred while processing payment. Please check your connection and try again.');
      } else if (error.message.includes('Invalid')) {
        throw error; // Pass through validation errors as-is
      }
    }
    
    throw new Error('An unexpected error occurred while processing the payment. Please try again or contact support.');
  }
};

export const waiveFine = async (token: string, fineId: number, waiveData: WaiveFinePayload): Promise<{ message: string; fine_id: string; waived_at: string; waived_by_admin: string; reason: string }> => {
  try {
    validateAuthToken(token);
    ...
    return await overdueApiRequest(
      ...
      () => apiRequest<{ message: string; fine_id: string; waived_at: string; waived_by_admin: string; reason: string }>(
        getApiUrl(url),
        {
          ...createAuthenticatedRequest(token),
          method: 'PUT',
    ...
    if (error instanceof Error) {
      if (error.message.includes('timeout')) {
        throw new Error('Fine waiver processing timed out. Please verify if the waiver was completed or try again.');
      } else if (error.message.includes('Authentication') || error.message.includes('401')) {
        throw new Error('Your session has expired. Please log in again to waive fines.');
      } else if (error.message.includes('403')) {
        throw new Error('You do not have permission to waive fines. This action requires administrator privileges.');
      } else if (error.message.includes('404')) {
        throw new Error('The fine record was not found. It may have already been processed or deleted.');
      } else if (error.message.includes('400')) {
        throw new Error('This fine cannot be waived. It may already be paid, waived, or in an invalid state.');
      } else if (error.message.includes('422')) {
        throw new Error('Invalid waiver information provided. Please check your reason and try again.');
      } else if (error.message.includes('Network') || error.message.includes('fetch')) {
        throw new Error('Network error occurred while processing waiver. Please check your connection and try again.');
      } else if (error.message.includes('Invalid') || error.message.includes('must be') || error.message.includes('required') || error.message.includes('cannot exceed')) {
        throw error; // Pass through validation errors as-is
      }
    }
...
export const getDashboardStats = async (token: string): Promise<{
  total_books: number;
  ...
  try {
    const statsData = await apiRequest<{
      ...
      getApiUrl(API_ENDPOINTS.DASHBOARD_STATS),
      createAuthenticatedRequest(token)
    );
...
export const getUserDashboardStats = async (token: string): Promise<{
  ...
  try {
    const response = await apiRequest<{
      ...
      getApiUrl(API_ENDPOINTS.USER_DASHBOARD_STATS),
      createAuthenticatedRequest(token)
    );

    // Validate response structure and provide defaults
    return {
      borrowed_books_count: typeof response.borrowed_books_count === 'number' ? response.borrowed_books_count : 0,
      available_books_count: typeof response.available_books_count === 'number' ? response.available_books_count : 0,
      overdue_books_count: typeof response.overdue_books_count === 'number' ? response.overdue_books_count : 0,
      total_fine_amount: typeof response.total_fine_amount === 'number' ? response.total_fine_amount : 0,
