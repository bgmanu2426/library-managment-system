...
# Constants for JWT
SECRET_KEY = os.getenv("SECRET_KEY", "09d25e094faa6ca2556c818166b7a9563b93f7099f6f0f4caa6cf63b88e8d3e7")  # Default fallback for development
...
def create_access_token(data: Dict[str, Any], expires_delta: Optional[timedelta] = None) -> str:
    ...
    logger.debug(f"Creating token for user_id={data.get('sub')} with expiration {expire}")
...
def verify_token(token: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug(f"Token successfully verified. Payload: user_id={payload.get('sub')}, exp={payload.get('exp')}")
        return payload
        ...
        logger.error(f"JWT verification error: {str(e)}, token: {token[:15]}...")
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED,
...
async def get_current_user(request: Request, token: Optional[str] = Depends(oauth2_scheme), session: Session = Depends(get_session)) -> User:
    ...
    logger.debug(f"Current user retrieved successfully: id={user.id}, email={user.email}, role={user.role}")
    return user

async def get_current_admin(current_user: User = Depends(get_current_user)) -> User:
    """Verify that the current user is an admin."""
    if current_user.role != "admin":
        logger.warning(f"Permission denied: User {current_user.id} ({current_user.email}) attempted admin access with role '{current_user.role}'")
        raise HTTPException(
